-- Reset users table and create superadmin (FIXED VERSION)
-- Run this in Supabase SQL Editor

-- 1. Drop the users table completely
DROP TABLE IF EXISTS public.users CASCADE;

-- 2. Recreate the users table with clean structure (without foreign key constraint to auth.users)
CREATE TABLE public.users (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    email TEXT UNIQUE NOT NULL,
    name TEXT NOT NULL,
    role TEXT NOT NULL CHECK (role IN ('superadmin', 'preschool_admin', 'teacher', 'parent')),
    preschool_id UUID, -- Removed FK constraint for now
    avatar_url TEXT,
    phone TEXT,
    is_active BOOLEAN DEFAULT true,
    auth_user_id UUID, -- Removed FK constraint to auth.users as it's causing issues
...
-- 3. DON'T enable RLS for now since you removed policies
...
-- 7. Insert the superadmin user directly (you already have the auth_user_id)
