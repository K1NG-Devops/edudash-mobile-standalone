-- Add missing onboarding_status column to preschools table
-- This fixes the school creation functionality
...
ALTER TABLE public.preschools ADD COLUMN IF NOT EXISTS tenant_slug varchar(50);
...
-- Create unique index for tenant_slug after adding the column
CREATE UNIQUE INDEX IF NOT EXISTS idx_preschools_tenant_slug_unique ON public.preschools(tenant_slug) WHERE tenant_slug IS NOT NULL;
...
-- Update the existing preschool records to have proper defaults
...
UPDATE public.preschools 
SET tenant_slug = lower(replace(replace(replace(name, ' ', '-'), '''', ''), '.', '')) || '-' || substring(id::text from 1 for 8)
WHERE tenant_slug IS NULL AND name IS NOT NULL;
