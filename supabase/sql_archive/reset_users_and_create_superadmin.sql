-- Reset users table and create superadmin
-- Run this in Supabase SQL Editor

-- 1. Drop the users table completely
DROP TABLE IF EXISTS public.users CASCADE;

-- 2. Recreate the users table with clean structure
...
-- 3. Enable RLS (but no policies for now since you removed them)
...
-- 6. Note: You'll need to manually create the auth user first in Supabase Auth
-- Then get the auth_user_id and insert the superadmin user record

-- After creating the auth user, run this insert (replace 'YOUR_AUTH_USER_ID' with actual ID):
