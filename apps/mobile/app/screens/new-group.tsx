...
export default function NewGroupScreen() {
  ...
  useEffect(() => {
    // Resolve auth user and preschool ID
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      const res = await ConversationService.createGroup({
        ...
        locked,
        allowMemberPosting,
        memberIds: [], // We can add a member picker later
      });
      if (res.error) throw new Error(res.error);
      // Optionally navigate straight into the new group chat later.
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.headerBtn, (!groupName.trim() || creating) && styles.headerBtnDisabled]} onPress={onCreate} disabled={!groupName.trim() || creating}>
          <Text style={[styles.headerBtnText, (!groupName.trim() || creating) && styles.headerBtnTextDisabled]}>{creating ? 'Creating...' : 'Create'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.content}>
        <View style={styles.formGroup}>
          ...
          <TextInput
            style={styles.input}
            placeholder="e.g. Butterfly Class Families"
            value={groupName}
        ...
        <View style={styles.sectionTitleRow}>
          <Text style={styles.sectionTitle}>Posting and controls</Text>
        </View>
        ...
        <View style={styles.footerNote}>
          <Text style={styles.footerText}>
            You’ll be the group owner. You can add members and manage settings after creating the group.
          </Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  footerNote: { marginTop: 16, borderTopWidth: 1, borderTopColor: '#334155', paddingTop: 12 },
