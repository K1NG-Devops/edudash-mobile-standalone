...
export default function AssignHomeworkScreen() {
  ...
  const classesQuery = useQuery({
    queryKey: ['teacher_classes', preschoolId],
    queryFn: async () => {
  ...
  const studentsQuery = useQuery({
    queryKey: ['students', preschoolId, classId],
    queryFn: async () => {
    ...
    enabled: !!preschoolId && mode === 'students',
    staleTime: 60_000,
  })
      ...
      <ScrollView contentContainerStyle={styles.contentPadding}>
        ...
        <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.outline }]}>
          ...
          <View style={styles.rowMv8}>
            <TouchableOpacity style={[styles.toggle, mode === 'class' && styles.toggleActive]} onPress={() => setMode('class')}>
              <Text style={[styles.toggleText, mode === 'class' && styles.toggleTextActive]}>Entire class</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.toggle, mode === 'students' && styles.toggleActive]} onPress={() => setMode('students')}>
              <Text style={[styles.toggleText, mode === 'students' && styles.toggleTextActive]}>Specific students</Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  toggle: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, borderWidth: StyleSheet.hairlineWidth, borderColor: '#CBD5E1', marginRight: 8 },
  toggleActive: { backgroundColor: '#3B82F6', borderColor: '#3B82F6' },
  toggleText: { color: '#111827', fontSize: 12, fontWeight: '600' },
