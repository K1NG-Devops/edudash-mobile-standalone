...
export const getAllOnboardingRequests = async (): Promise<Partial<PreschoolOnboardingRequest>[]> => {
  // Prefer secure Edge Function (service role on server) to avoid client-side RLS issues
  try {
    const { data, error } = await supabase.functions.invoke('superadmin_get_onboarding_requests', {
      body: {},
    });
    if (error) {
      log.error('Edge function superadmin_get_onboarding_requests failed:', error);
      throw new Error('Failed to load onboarding requests');
    }
    return (data?.requests || []) as Partial<PreschoolOnboardingRequest>[];
  } catch (e) {
    // Fallback: attempt direct select if Edge Functions are unavailable (dev only)
    const client = supabaseAdmin || supabase;
    const { data, error } = await client
      .from('preschool_onboarding_requests')
      .select('*')
