...
export class SuperAdminDataService {
  ...
  static async getSuperAdminDashboardData(superAdminUserId: string): Promise<SuperAdminDashboardData> {
    try {
      console.log('🔍 [SuperAdminDataService] Starting getSuperAdminDashboardData for userId:', superAdminUserId);

      // Verify super admin permissions
      console.log('🔐 [SuperAdminDataService] Verifying super admin permissions...');
      const hasPermission = await this.verifySuperAdminPermissions(superAdminUserId);
      console.log('🔐 [SuperAdminDataService] Permission check result:', hasPermission);
      if (!hasPermission) {
        throw new Error('Access denied: Super admin permissions required');
      }

      console.log('📦 [SuperAdminDataService] Starting parallel data fetch...');
      ...
      const [
        ...
        this.getPlatformStats().then(result => {
          console.log('✅ [SuperAdminDataService] getPlatformStats completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getPlatformStats failed:', err);
          throw err;
        }),
        this.getRecentSchools().then(result => {
          console.log('✅ [SuperAdminDataService] getRecentSchools completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getRecentSchools failed:', err);
          throw err;
        }),
        this.getRecentUsers().then(result => {
          console.log('✅ [SuperAdminDataService] getRecentUsers completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getRecentUsers failed:', err);
          throw err;
        }),
        this.getPlatformActivity().then(result => {
          console.log('✅ [SuperAdminDataService] getPlatformActivity completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getPlatformActivity failed:', err);
          throw err;
        }),
        this.getSystemHealth().then(result => {
          console.log('✅ [SuperAdminDataService] getSystemHealth completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getSystemHealth failed:', err);
          throw err;
        }),
        this.getPendingApprovals().then(result => {
          console.log('✅ [SuperAdminDataService] getPendingApprovals completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getPendingApprovals failed:', err);
          throw err;
        }),
        this.getSystemAlerts().then(result => {
          console.log('✅ [SuperAdminDataService] getSystemAlerts completed');
          return result;
        }).catch(err => {
          console.error('❌ [SuperAdminDataService] getSystemAlerts failed:', err);
          throw err;
        })
      ]);
  ...
  static async verifySuperAdminPermissions(userId: string): Promise<boolean> {
    try {
      console.log('🔐 [SuperAdmin] Starting permission verification for userId:', userId);
      ...
      try {
        console.log('🔍 [SuperAdmin] Checking JWT metadata for role...');
        const { data: auth } = await supabase.auth.getUser();
        const authUser = auth?.user;
        console.log('🔍 [SuperAdmin] Auth user found:', {
          hasUser: !!authUser,
          userId: authUser?.id,
          targetUserId: userId,
          userMatches: authUser?.id === userId
        });
        ...
        if (authUser?.id === userId) {
          ...
          console.log('🔍 [SuperAdmin] User metadata role check:', {
            metadata: md,
            claimRole,
            isSuperadmin: String(claimRole).toLowerCase() === 'superadmin'
          });
          
          if (String(claimRole).toLowerCase() === 'superadmin') {
            console.log('✅ [SuperAdmin] Permission verified via JWT metadata');
            return true;
          }
        }
      } catch (e) {
        console.warn('⚠️ [SuperAdmin] JWT check failed, falling back to DB:', e);
        // Non-fatal; fall through to DB-based check
      }

      // 2) Fallback to DB check (requires users self-select policy)
      console.log('🔍 [SuperAdmin] Falling back to database role check...');
      ...
      console.log('🔍 [SuperAdmin] Database user lookup result:', {
        hasUser: !!user,
        role: user?.role,
      ...
      const hasPermission = user.role === 'superadmin' && !!user.is_active;
      console.log(hasPermission ? '✅' : '❌', '[SuperAdmin] Final permission result:', hasPermission);
  ...
  static async getPendingApprovals() {
    try {
      ...
      // Attempt count safely; if table/policy missing, fall back to 0.
