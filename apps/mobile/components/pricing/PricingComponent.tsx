...
// Unified flow: use PayFast via createSubscription for all platforms
...
export const PricingComponent = ({
  ...
  // Auth-aware pricing: if user is logged in, use authenticated flows; otherwise, fall back to public view
  ...
  const [rcOfferingLoaded, setRcOfferingLoaded] = useState(true);
  ...
  // RevenueCat flow removed to unify payments across platforms via PayFast
  useEffect(() => {
...
'Create up to 25 AI-powered lessons monthly',
  ...
  const handleSelectPlan = async (plan: PricingPlan) => {
    // Unified: always use the server-side subscription flow (PayFast) for all platforms
  ...
  const processPlanSelection = async (plan: PricingPlan, role: string) => {
    // Resolve the server plan id from loaded plans; fallback to plan.id (slug)
    const serverPlan = plans.find(p => p.tier === plan.value);
    const planIdToSend = serverPlan?.id || plan.id;
    ...
    try {
      ...
      if (result.success && (result.payment_url || result.approval_url)) {
        ...
        const target = result.payment_url || result.approval_url;
