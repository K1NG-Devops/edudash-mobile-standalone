...
import { displayNameForTier } from '@/lib/utils/tiers';
...
export const DashboardSubscriptionCard: React.FC<DashboardSubscriptionCardProps> = ({
  ...
  const planTier = (subscription?.plan as any)?.tier as string | undefined;
  const planDisplayName =
    displayNameForTier(planTier, subscription?.plan?.name) ??
    i18n.t('subscription.plan.freePlan', { defaultValue: 'Free' });
      ...
      >
        <View style={styles.compactContent}>
          ...
          <Text style={[styles.compactPlan, { color: palette.text }]}>
            {displayNameForTier(planTier, subscription?.plan?.name) || i18n.t('subscription.plan.freePlan', { defaultValue: 'Free' })}
          </Text>
          {isFreeTier && (
            <LinearGradient
