...
class AuthProviderClass extends React.Component<AuthProviderProps, AuthProviderState> {
  ...
  loadProfile = async (userId: string) => {
    try {
      ...
      // If no data found, build a minimal profile from JWT so UI can proceed (web/dev-safe)
      if (!data) {
        console.log('⚠️ [DEBUG] No profile found for auth_user_id:', userId);
        const minimal = await this.buildMinimalProfileFromAuth();
        if (minimal) {
          console.log('ℹ️ [DEBUG] Using minimal profile from JWT metadata. Role:', minimal.role);
          this.setState({ profile: minimal, loading: false });
          return;
        }
      }

      // Set profile to null if nothing could be derived
      this.setState({
        profile: null,
        loading: false
      });
