...
export class TeacherDataService {
   ...
   *
   * All lookups are RLS-aware; teacherUserId is the auth_user_id of the teacher.
   */
  static async assignLesson(
    ...
    try {
      // Get teacher profile and preschool context
      ...
      const { error: insErr } = await supabase.from('homework_submissions').insert(rows as any)
      if (insErr) {
      ...
      log.error('❌ assignLesson unexpected error:', e)
